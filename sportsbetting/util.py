...
from .models import Participant, Sport, Ticket


def read_uploaded_ticket(
	request: HttpRequest, ticket: Ticket, end_date: datetime.date
) -> None:
	if ticket._state.adding:
		messages.error(
			request,
			"Error: read_uploaded_ticket() was passed an unsaved model instance",
		)
		return
	...
	try:
		...
		with open(f"{settings.MEDIA_ROOT}/ticket.csv", mode="r") as source:
			...
			for row in reader:
				...
				if (
					not entry_sport
					or not entry_first_team
					or not entry_second_team
					or not entry_commence_time
				):
				...
				try:
					...
					messages.error(
						f"Error: Could not look up sport {entry_sport}; is it present in the database?"
					)
					ticket.delete()
					return
				else:
				...
				try:
					try:
						date_and_time = timezone.make_aware(
							datetime.datetime.fromisoformat(entry_commence_time)
						)
					except ValueError:
						date_and_time = timezone.make_aware(
							datetime.datetime.strptime(
								entry_commence_time.replace("-", "/"),
								"%m/%d/%y %H:%M:%S",
							)
						)
				except ValueError:
					messages.error(
						request,
						f"Error: Invalid commence_time on line {line} (got '{entry_commence_time}', but format should follow ISO format; example: '2011-11-04T00:05:23')",
					)
					ticket.delete()
					return
				...
				try:
					...
					messages.error(
						request,
						f"Error: Invalid spread on line {line} (got '{row.get('spread')}', but should be numeric integer only or numeric integer prefixed with 'P')",
					)
					ticket.delete()
					return
				...
				if spread_entry and (entry_under or entry_over):
					try:
						...
						messages.error(
							request,
							f"Error: Invalid under/over values on line {line} of the spreadsheet",
						)
						ticket.delete()
						return
			...
			if end_date:
				...
				if end_date < last_greatest_date:
					messages.warning(
						request,
						f"Warning: Games on ticket '{ticket.name}' conclude on '{last_greatest_date}', but the user specified end date is '{end_date}'",
					)
			else:
			...
			if end_date >= ticket.pub_date:
				...
				messages.success(
					request,
					f"Success: Parsed spreadsheet and created ticket '{ticket.name}'",
				)
				return
			else:
				messages.error(
					request,
					f"Error: All games for ticket '{ticket.name}' begin before the publish date, or the specified end date is earlier than the publish date",
				)
				ticket.delete()
				return
	except IOError:
		messages.error(
			request,
			"Error: Could not read ticket file on the server (are permissions configured properly on the server?)",
		)
		ticket.delete()
		return
...
def generate_ticket_spreadsheet(
	modeladmin: ModelAdmin, request: HttpRequest
) -> HttpResponse:
	try:
		with open(f"{settings.MEDIA_ROOT}/generated.csv", mode="w") as f:
			...
			for sport in request.POST.keys():
				...
				for i in data:
					...
					home_team = Participant.objects.get_or_create(
						type="TM",
						sport=sport,
						name=i.get("home_team").encode("ascii", "ignore").decode(),
					)[0]
					away_team = Participant.objects.get_or_create(
						type="TM",
						sport=sport,
						name=i.get("away_team").encode("ascii", "ignore").decode(),
					)[0]
					...
					try:
						...
						messages.warning(
							request,
							f"Some objects from the API provider's response couldn't be parsed (api_id: {i.get('id')})",
						)
