        ...
        with StringIO(file_content) as source:
            ...
            for line, row in enumerate(reader, start=1):
                try:
                    ...
                    governing_body_id_or_name = row.get("governing_body", "").strip()
                    ...
                    # Governing body resolution logic
                    if not governing_body_id_or_name:
                        # Try to resolve via home team
                        try:
                            home_team_obj = Team.objects.filter(
                                name__iexact=home_team_name
                            )
                            if home_team_obj.exists():
                                governing_body = home_team_obj.first().governing_body
                            else:
                                # Try to resolve via sport
                                governing_bodies = GoverningBody.objects.filter(
                                    sport=sport
                                )
                                if governing_bodies.count() == 1:
                                    governing_body = governing_bodies.first()
                                else:
                                    # TODO: Fetch from API if available
                                    messages.error(
                                        request,
                                        _(
                                            f"Could not resolve governing body for line {line}. Please specify or add to DB."
                                        ),
                                    )
                                    raise KeyError("governing_body")
                            ...
                            raise KeyError("governing_body") from e
                    else:
