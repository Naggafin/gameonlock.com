...
from .util import read_uploaded_ticket, parse_ticket, generate_ticket_page, generate_ticket_spreadsheet
from .models import Ticket, SpreadEntry, UnderOverEntry, TicketPlay, SpreadPick, UnderOverPick, Participant, Sport
...
class UploadTicketForm(forms.Form):
	name = forms.CharField(label="Ticket name", max_length=255, initial=settings.SPORTS['default_ticket_name'])
	date = forms.DateField(label="Publish date", initial=datetime.date.today())
	end_date = forms.DateField(label="End date", required=False, help_text="If left blank, an end date for the ticket will be calculated automatically")
...
class TicketAdmin(admin.ModelAdmin):
	...
	def change_view(self, request, object_id, form_url='', extra_context=None):
		...
		extra_context['form'] = ChangeTicket(initial={'object_id': object_id, 'end_date': old_ticket.end_date})
		return super(TicketAdmin, self).change_view(request, object_id, form_url, extra_context)
	
	def get_urls(self):
		urls = super().get_urls()
		my_urls = [
			path('upload_ticket/', self.admin_site.admin_view(self.upload_ticket), name='upload_ticket'),
			path('change_ticket/', self.admin_site.admin_view(self.change_ticket), name='change_ticket'),
			path('generate_ticket/', self.admin_site.admin_view(self.generate_ticket), name='generate_ticket'),
		]
		return my_urls + urls
	...
	def upload_ticket(self, request):
		if request.method == 'POST':
			...
			if form.is_valid():
				...
				try:
					ticket.save()
					read_uploaded_ticket(request, ticket, form.cleaned_data.get('end_date'))
				except IntegrityError:
					messages.error(request, f"Error: A ticket with a pub_date of {form.cleaned_data.get('date')} already exists, or invalid data was submitted")
	...
	def change_ticket(self, request):
		if request.method == 'POST':
			...
			if form.is_valid():
				try:
					old_ticket = Ticket.objects.get(pk=form.cleaned_data.get('object_id'))
				except ObjectDoesNotExist:
					messages.error(request, f"Error: It seems the given ticket does not exist (tried to look up by ID {form.get('object_id')})")
				else:
					new_ticket = Ticket(
					...
					if parse_ticket(request, new_ticket, end_date):
						try:
							...
							messages.error(request, f"Error: Unable to save clone of ticket {form.cleaned_data.get('object_id')}")
						else:
...
class TicketPlayAdmin(admin.ModelAdmin):
	...
	actions = ['set_paid', 'set_unpaid', 'set_won', 'unset_won',]
